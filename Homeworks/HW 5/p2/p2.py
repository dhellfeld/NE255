...
while (outerconverge == False):
    ...
    for group in range(3):
        groupsource += q_e[group] + (2./18.) * (sig_s[group,0]*psi_group1.sum(axis=1)) + (sig_s[group,1]*psi_group2.sum(axis=1)) + (sig_s[group,2]*psi_group3.sum(axis=1))
    ...
    for group in range(3):
        ...
        while (innerconverge == False):
            ...
            for i in range(int(N)):
                ...
                s = q_e[group] + (2./18.) * ((sig_s[group,0]*psi_group1[i,:].sum()) + (sig_s[group,1]*psi_group2[i,:].sum()) + (sig_s[group,2]*psi_group3[i,:].sum()))
            ...
            # Sweep in mu < 0 (left)
            for i in range(int(N)):
                s = q_e[group] + (2./18.) * ((sig_s[group,0]*psi_group1[N-i-1,:].sum()) + (sig_s[group,1]*psi_group2[N-i-1,:].sum()) + (sig_s[group,2]*psi_group3[N-i-1,:].sum()))
    ...
    # Calculate new groupsource values
    groupsource_new = np.zeros(N)
    for group in range(3):
        groupsource_new += q_e[group] + (2./18.) * (sig_s[group,0]*psi_group1.sum(axis=1)) + (sig_s[group,1]*psi_group2.sum(axis=1)) + (sig_s[group,2]*psi_group3.sum(axis=1))
